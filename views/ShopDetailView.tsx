...
interface ShopDetailViewProps {
  ...
  onAddTask: (shopId: string, type: TaskType, dueDate: string) => void;
...
export const ShopDetailView: React.FC<ShopDetailViewProps> = ({
  ...
  const handleAddTask = (e: React.FormEvent) => {
    ...
    onAddTask(shop.id, newTaskType, newTaskDate);
    setNewTaskDate('');
  };
        ...
        <div className="space-y-6">
          ...
          {tasks.map(task => (
             <div key={task.id} className="bg-white p-4 rounded-2xl border border-gray-100 flex items-center gap-4">
                ...
                <div className="flex-1">
                  <div className="font-extrabold text-sm">{task.type}</div>
                  <div className="text-[11px] font-bold opacity-60">Due: {task.dueDate}</div>
                </div>
